...
import {Paper, InputBase, InputAdornment, Select, MenuItem, Divider, makeStyles} from '@material-ui/core';
...
function SearchForm({query: queryParam, queryType: queryTypeParam, onQueryChange, onQueryTypeChange, onSubmit, className}) {
  const classes = useStyles();
  className = className || {};

  const history = useHistory()
  const [ query, setQuery ] = useState(queryParam || '');
  const [ queryType, setQueryType ] = useState(queryTypeParam || QUERY_TYPE.ARTIST);
